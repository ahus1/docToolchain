...
It then creates an AsciiDoc file that includes all files found.

When you create a modular documentation, most includes are static.
For example, the arc42-template has 12 chapters and one master template that includes those 12 chapters.

When you work with dynamic modules like ADRs (Architecture Decision Records), you create those files on the fly.
Maybe not even within your `/src/docs` folder but right beside the code file for which you wrote the ADR.

In order to include these files in your documentation, you would have to add the file with its whole relative path to one of your AsciiDoc files.

This task will handle it for you!

Just stick with your filenames to the pattern `+++^[A-Z]{3,}-.*+++` (it begins with at last three upper case letters and a dash) and this task will collect this file and write it to your build folder.
...
Obviously, you get the most benefit if you not only have one ADR file, but several get collected.
😎
